# Google Ads Kampagnenplan - Rohrreinigung Kraft (Amberg)

## Übersicht
- **Unternehmen:** Rohrreinigung Kraft
- **Filiale:** Amberg (eigener Standort vor Ort)
- **Zielregion dieser Kampagne:** Amberg + 60km Umkreis (Oberpfalz)
- **Ziel:** Lead-Generierung (Anrufe + Kontaktformulare)
- **Budget-Empfehlung:** 30-50€/Tag
- **Hinweis:** Dank eigener Filiale in Amberg können alle Anzeigen mit
  schneller, lokaler Anfahrt werben (ca. 20-40 Min in Amberg selbst).

---

## Kampagne 1: Notdienst (Höchste Priorität)

### Einstellungen
- **Name:** Rohrreinigung Notdienst 24/7
- **Typ:** Suchnetzwerk
- **Zielregion:** Amberg, Kümmersbruck, Ammerthal, Sulzbach-Rosenberg + 50km Umkreis
- **Budget:** 20€/Tag
- **Gebotsstrategie:** Conversions maximieren

### Anzeigengruppe 1: Rohr verstopft Notfall
**Keywords:**
- rohrreinigung notdienst
- rohr verstopft notfall
- abfluss verstopft notdienst
- klempner notdienst amberg
- toilette verstopft hilfe
- verstopfung notdienst
- 24 stunden rohrreinigung
- rohrreinigung sofort
- notdienst abfluss
- kanalreinigung notdienst

**Anzeige 1:**
```
Überschrift 1: Rohrverstopfung? Sofort-Hilfe!
Überschrift 2: 24/7 Notdienst Amberg
Überschrift 3: In 20-40 Min vor Ort
Beschreibung 1: Professionelle Rohrreinigung rund um die Uhr. Faire Preise. Jetzt anrufen!
Beschreibung 2: Lokaler Fachbetrieb ✓ Moderne Technik ✓ Keine versteckten Kosten ✓
Finale URL: https://rohrreinigung-kraft-amberg.de
Anruf-Erweiterung: 0178 7401958
```

**Anzeige 2:**
```
Überschrift 1: Abfluss verstopft? Wir helfen!
Überschrift 2: Notdienst 24/7 - Amberg
Überschrift 3: ☎ 0178 7401958
Beschreibung 1: Rohrreinigung Notdienst - Schnelle Hilfe bei Verstopfungen. Professionell & zuverlässig.
Beschreibung 2: 10+ Jahre Erfahrung ✓ 2000+ Einsätze ✓ 98% Kundenzufriedenheit ✓
Finale URL: https://rohrreinigung-kraft-amberg.de/service/rohrreinigung-notdienst
```

### Anzeigengruppe 2: Toilette verstopft
**Keywords:**
- toilette verstopft
- wc verstopft
- toilette verstopfung
- toilette verstopft was tun
- klo verstopft amberg
- toilette abfluss verstopft

**Anzeige:**
```
Überschrift 1: Toilette verstopft?
Überschrift 2: Schnelle Hilfe in Amberg
Überschrift 3: 24/7 Notdienst
Beschreibung 1: WC verstopft? Wir helfen professionell und ohne Schäden.
Beschreibung 2: Faire Preise ✓ Keine Anfahrtskosten ✓ Saubere Arbeit ✓
Finale URL: https://rohrreinigung-kraft-amberg.de/service/toilette-verstopft
```

---

## Kampagne 2: Rohrreinigung Allgemein

### Einstellungen
- **Name:** Rohrreinigung Services
- **Typ:** Suchnetzwerk
- **Zielregion:** Amberg + 60km Umkreis
- **Budget:** 15€/Tag
- **Gebotsstrategie:** Conversions maximieren

### Anzeigengruppe 1: Rohrreinigung
**Keywords:**
- rohrreinigung amberg
- rohrreinigung kümmersbruck
- rohrreinigung sulzbach-rosenberg
- abflussreinigung
- rohr reinigen lassen
- verstopftes rohr
- rohrreinigung kosten
- professionelle rohrreinigung

**Anzeige:**
```
Überschrift 1: Rohrreinigung Amberg
Überschrift 2: Professionell & Fair
Überschrift 3: Kostenlose Beratung
Beschreibung 1: Rohrreinigung vom Fachbetrieb. Modernste Technik für alle Verstopfungen.
Beschreibung 2: Termine nach Absprache ✓ Transparente Preise ✓ Lokaler Service ✓
Finale URL: https://rohrreinigung-kraft-amberg.de/service/rohrreinigung
```

### Anzeigengruppe 2: Kanalreinigung
**Keywords:**
- kanalreinigung amberg
- kanal reinigen
- kanalreinigung kosten
- abwasserkanal reinigung
- hochdruckreinigung kanal
- kanalspülung

**Anzeige:**
```
Überschrift 1: Kanalreinigung Amberg
Überschrift 2: Hochdruck-Spülung
Überschrift 3: Jetzt Termin vereinbaren
Beschreibung 1: Professionelle Kanalreinigung mit modernster Hochdrucktechnik. Schnell & gründlich.
Beschreibung 2: TV-Inspektion möglich ✓ Wartungsverträge ✓ Faire Preise ✓
Finale URL: https://rohrreinigung-kraft-amberg.de/service/kanalreinigung
```

### Anzeigengruppe 3: Kamera-Inspektion
**Keywords:**
- kamerainspektion rohr
- kanalinspektion
- tv inspektion kanal
- rohr kamera
- kanal kamera untersuchung

**Anzeige:**
```
Überschrift 1: Rohr-Kamerainspektion
Überschrift 2: Schäden schnell finden
Überschrift 3: In Amberg & Umgebung
Beschreibung 1: TV-Inspektion für Rohre & Kanäle. Präzise Schadensanalyse mit modernster Technik.
Beschreibung 2: Dokumentation inklusive ✓ Sofortige Auswertung ✓
Finale URL: https://rohrreinigung-kraft-amberg.de/service/kamera-inspektion
```

---

## Kampagne 3: Städte-Targeting

### Einstellungen
- **Name:** Rohrreinigung Städte
- **Typ:** Suchnetzwerk
- **Budget:** 10€/Tag
- **Gebotsstrategie:** Klicks maximieren

### Anzeigengruppe je Stadt:
Für jede Stadt eine separate Anzeigengruppe mit lokalen Keywords:

**Städte:** Kümmersbruck, Ammerthal, Sulzbach-Rosenberg, Hahnbach, Vilseck, Hirschau, Freudenberg, Schnaittenbach, Auerbach i.d.OPf., Kastl, Königstein, Neumarkt i.d.OPf., Schwandorf, Weiden i.d.OPf., Regensburg

**Beispiel Anzeigengruppe: Sulzbach-Rosenberg**
**Keywords:**
- rohrreinigung sulzbach-rosenberg
- abfluss verstopft sulzbach-rosenberg
- klempner sulzbach-rosenberg
- notdienst rohrreinigung sulzbach-rosenberg

**Anzeige:**
```
Überschrift 1: Rohrreinigung Sulzbach-Rosenberg
Überschrift 2: Zuverlässiger Service
Überschrift 3: 24/7 erreichbar
Beschreibung 1: Rohrreinigung in Sulzbach-Rosenberg & Umgebung. Lokaler Fachbetrieb.
Beschreibung 2: Notdienst verfügbar ✓ Faire Preise ✓ Moderne Technik ✓
Finale URL: https://rohrreinigung-kraft-amberg.de/sulzbach-rosenberg
```

---

## Anzeigenerweiterungen

### Sitelink-Erweiterungen
1. **Notdienst 24/7** → /service/rohrreinigung-notdienst
2. **Alle Leistungen** → /leistungen
3. **Kontaktformular** → /kontakt
4. **Einsatzgebiete** → /staedte

### Anruf-Erweiterung
- Telefon: 0178 7401958
- Anzeige: "Jetzt anrufen"

### Standort-Erweiterung
- Amberg & 60km Umkreis

### Snippet-Erweiterungen
- **Services:** Rohrreinigung, Kanalreinigung, Notdienst, Kamera-Inspektion
- **Vorteile:** 24/7 Erreichbar, Faire Preise, Schnelle Anfahrt, Lokaler Betrieb

### Callout-Erweiterungen
- ✓ 24/7 Notdienst
- ✓ In 20-40 Min vor Ort
- ✓ Keine versteckten Kosten
- ✓ 10+ Jahre Erfahrung
- ✓ Kostenlose Beratung

---

## Negative Keywords (Für alle Kampagnen)

```
- selber machen
- selbst
- diy
- anleitung
- hausmittel
- kostenlos
- gratis
- job
- jobs
- stellenangebot
- ausbildung
- gehalt
- münchen
- berlin
- hamburg
- köln
- frankfurt
- nürnberg
- fürth
- erlangen
```

Hinweis: `nürnberg`, `fürth`, `erlangen` wurden bewusst als Negative Keywords
ergänzt, da dieselbe Google Ads-Kontonummer (789-424-2096) auch die
Nürnberg-Kampagne des Unternehmens betreibt — so vermeiden die beiden
Kampagnen, um dieselben Suchanfragen zu konkurrieren.

---

## Conversion-Tracking

### Primäre Conversions
1. **Anruf-Conversion**
   - Typ: Anruf aus Anzeige
   - Wert: 30€
   - Zählung: Einmal

2. **Formular-Conversion**
   - Typ: Website-Aktion
   - Seite: /thank-you
   - Wert: 50€
   - Zählung: Einmal

3. **WhatsApp-Klick**
   - Typ: Website-Aktion
   - Event: whatsapp_click
   - Wert: 20€

### Sekundäre Conversions
- Telefon-Klick auf Website
- Kontaktseite besucht
- Service-Seite besucht

---

## Budget-Verteilung

| Kampagne | Tagesbudget | Monatlich |
|----------|-------------|-----------|
| Notdienst | 20€ | ~600€ |
| Services | 15€ | ~450€ |
| Städte | 10€ | ~300€ |
| **Gesamt** | **45€** | **~1.350€** |

---

## Optimierungstipps

1. **Erste 2 Wochen:** Daten sammeln, keine großen Änderungen
2. **Nach 2 Wochen:** Schwache Keywords pausieren (CTR < 2%)
3. **Nach 4 Wochen:** Gebotsanpassungen basierend auf Conversion-Daten
4. **Monatlich:** Suchanfrageberichte prüfen, neue negative Keywords hinzufügen
5. **Saisonal:** Budget im Winter erhöhen (mehr Verstopfungen durch Frost)

---

## Import in Google Ads Editor

Die CSV-Datei `google-ads-import.csv` kann direkt in Google Ads Editor importiert werden:
1. Google Ads Editor öffnen
2. Konto auswählen
3. "Import" → "Aus Datei importieren"
4. CSV-Datei auswählen
5. Änderungen überprüfen
6. "Posten" klicken
